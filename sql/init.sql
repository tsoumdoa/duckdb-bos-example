...
-- unit enum table
INSERT INTO enum_unit (symbol, category, base_symbol, conversion_factor, "offset")
VALUES
  -- Length
  ('m', 'length', 'm', 1.0, 0.0),
...
AS
select LocalId, GlobalId,Entities."index" as index, Entities.name as entity_name,  s_name.Strings as name, s_category.Strings as category,  s_path.Strings as path_name, s_title.Strings as project_name 
from Entities
...
AS SELECT dsc.index as index, strName.Strings as Name, strUnit.Strings as Units, strGroup.Strings as Group, strType.Strings as Type
